...
<template>
  <div class="shoppingCart">
    ...
    <!-- 购物车主要内容区 -->
    <div class="content" v-if="shoppingCartList.length>0">
      <ul>
        <!-- 购物车表头 -->
        <li class="header">
          <div class="pro-check">
        ...
        <!-- 购物车列表 -->
        <li class="product-list" v-for="(item,index) in shoppingCartList" :key="index">
          <div class="pro-check">
            <el-checkbox
          ...
          <div class="pro-total pro-total-in">{{item.productPrice*item.totalNum}}元</div>
...
export default {
  ...
  methods: {
    ...
    //获取数据
    async getData(){
      await this.$store.dispatch('getShoppingCartList', {user_id:this.$store.getters.getUser.userId})
    },
    // 修改商品数量的时候调用该函数
    ...
    async deleteItem(a,b,productId) {
      await this.$store.dispatch('deleteShoppingCart',{
        user_id: this.$store.getters.getUser.userId,
        product_id: productId
      })
          .then(() => {
    ...
    async updateAllChecked(event) {
      ...
      try {
        await this.$axios.post('/api/shoppingCart/updateAllCheck',{user_id: this.$store.getters.getUser.userId, is_checked})
        await this.getData();
      } catch (error) {
        alert(error.message);
      }
